 ...
 * COSNTANTS
 **********************************************************************************************************************/
static const char *TAG_SYS = "SUSTEM";
/***********************************************************************************************************************
 * MACROS
 **********************************************************************************************************************/
...
/***********************************************************************************************************************
 * @brief Responsible for the initialization of the File System
 *
 * @param
 *
 * @return void
...
esp_err_t init_fs(void) {
    ...
    if (ret != ESP_OK) {
        if (ret == ESP_FAIL) {
            ESP_LOGE(TAG_SYS, "Failed to mount or format filesystem");
        } else if (ret == ESP_ERR_NOT_FOUND) {
            ESP_LOGE(TAG_SYS, "Failed to find SPIFFS partition");
        } else {
            ESP_LOGE(TAG_SYS, "Failed to initialize SPIFFS (%s)", esp_err_to_name(ret));
        }
        return ESP_FAIL;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG_SYS, "Failed to get SPIFFS partition information (%s).", esp_err_to_name(ret));
    } else {
        ESP_LOGI(TAG_SYS, "Partition size: total: %d, used: %d", total, used);
    }
    return ESP_OK;
}
